...
function initCommunicationSimulator() {
    ...
    // ⚡ Bolt Optimization: Use lookup object for O(1) results access instead of nested O(n) if-else chain
    ...
    simulateButton.addEventListener('click', function() {
        ...
        // ⚡ Bolt: using O(1) lookup
        let results = '';
        if (style === 'Mixed') {
             // Fallback for mixed or undefined styles if needed, though previously it just showed empty
             results = '<p class="mb-4">Please adjust the sliders to more clearly define a style.</p>';
        } else if (simulationData[scenarioKey] && simulationData[scenarioKey][style]) {
            results = simulationData[scenarioKey][style];
        }
