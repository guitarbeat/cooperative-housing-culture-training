...
function initCommunicationSimulator() {
    ...
    // Define scenarios
    const scenarios = COMMUNICATION_SCENARIOS;
    ...
    simulateButton.addEventListener('click', function() {
        ...
        const results = SIMULATION_RESULTS[scenarioKey]?.[style] || '';
...
function initPolicyWizard() {
    ...
    // Use shared POLICY_TYPES object
    const policyTypes = POLICY_TYPES;

    // Add a button to generate the policy, initially hidden
    // Check if button already exists to prevent duplication on re-init
